...
enum class Settings constructor(private val key: String, defaultValue: Any = "") {
    ...
    fun isDefined() = PREFS.retrieve(key) != null

    fun get() = PREFS.retrieve(key) ?: defaultVal
    fun getOr(alternative: String) = PREFS.retrieve(key) ?: alternative
    fun getBool(): Boolean = get() == "true"

    fun put(value: String) = PREFS.store(key, value)
    fun put(value: Any) = PREFS.store(key, value.toString())
